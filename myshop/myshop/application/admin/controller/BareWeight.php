...
use think\Request;
use app\admin\model\BareWeight as BareWeightModel;
use think\Session;
 ...
 */
class BareWeight extends Controller
{
    /**
     * 品牌添加,修改数据
     * $id：修改的条件
    ...
    public function save($id=null)//$id：修改的条件
    {
        ...
        if(is_null($id)){
            $model = new BareWeightModel();//实例化模型-添加数据
        }
        else{
            $model = BareWeightModel::get($id);//修改数据-实例化模型
        }
    ...
    {   #/a:返回的是数组
        BareWeightModel::destroy(input('selected/a',[]));
        $this->redirect('index');//跳转到查看页面
    }
