...
use think\Request;
use app\admin\model\RunningMemory as RunningMemoryModel;
use think\Session;
 ...
 */
class RunningMemory extends Controller
{
    /**
     * 品牌添加,修改数据
     * $id：修改的条件
    ...
    public function save($id=null)//$id：修改的条件
    {
        ...
        if(is_null($id)){
            $model = new RunningMemoryModel();//实例化模型-添加数据
        }
        else{
            $model = RunningMemoryModel::get($id);//修改数据-实例化模型
        }
        ...
        elseif ($request->isPost()) {
            ...
            $validate = validate('RunningMemory');
    ...
    {   #/a:返回的是数组
        RunningMemoryModel::destroy(input('selected/a',[]));
        $this->redirect('index');//跳转到查看页面
    }
