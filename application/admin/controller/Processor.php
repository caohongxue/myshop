...
use think\Request;
use app\admin\model\Processor as ProcessorModel;
use think\Session;
...
class Processor extends Controller
{
    ...
    public function save($id=null)//$id：修改的条件
    {
        ...
        if(is_null($id)){
            $model = new ProcessorModel();//实例化模型-添加数据
        }
        else{
            $model = ProcessorModel::get($id);//修改数据-实例化模型
        }
    ...
    {   #/a:返回的是数组
        ProcessorModel::destroy(input('selected/a',[]));
        $this->redirect('index');//跳转到查看页面
    }
