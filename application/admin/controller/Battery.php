...
use think\Request;
use app\admin\model\Battery as BatteryModel;
use think\Session;
...
class Battery extends Base
{
    ...
    public function save($id=null)//$id：修改的条件
    {
        ...
        if(is_null($id)){
            $model = new BatteryModel();//实例化模型-添加数据
        }
        else{
            $model = BatteryModel::get($id);//修改数据-实例化模型
        }
    ...
    {   #/a:返回的是数组
        BatteryModel::destroy(input('selected/a',[]));
        $this->redirect('index');//跳转到查看页面
    }
