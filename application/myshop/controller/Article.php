...
//        var_dump($data);die;
        ...
        $goods_d=Db::table('goods')->where(['brand_id'=>$id])->select();
        $all_d=Db::table('goods')
            ->alias('a')
        ...
        $user_comment__all_count_d=Db::table('comment')
            ->alias('a')
            ->join('user b','a.user_id=b.user_id')
            ->join('goods c','a.goods_id=c.goods_id')
            ->where(['a.goods_id'=>$id,'a.static'=>1])
            ->count();
        //全部评价
        $user_comment_all_d=Db::table('comment')
            ->alias('a')
            ->join('user b','a.user_id=b.user_id')
            ->join('goods c','a.goods_id=c.goods_id')
            ->where(['a.goods_id'=>$id,'a.static'=>1])
            ->paginate(5);
        $maybe_love=Db::table('goods')
            ->order('click_name','sale_number')
            ->paginate(8);
//        var_dump($maybe_love);die;
        return $this->fetch('each',['data'=>$data,'brand'=>$brand,'pro'=>$pro,'run'=>$run,'mem'=>$mem,'you'=>$you,'goods_d'=>$goods_d,'all_d'=>$all_d,'user_comment_all_d'=>$user_comment_all_d,'user_comment__all_count_d'=>$user_comment__all_count_d,'maybe_love'=>$maybe_love]);
    }
